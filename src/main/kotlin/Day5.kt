class Day5(input: String? = null) : AdventOfCodeDay(input) {
    ...
    private fun Sequence<Long>.toLocation(stages: List<Stage>) = map { input ->
        stages.fold(input) { value, stage ->
            stage.mappings.firstOrNull { it.range.contains(value) }?.let { value + it.increment } ?: value
        }
    }
    ...
    private fun LongRange.mapInto(mappings: List<Mapping>) = sequence {
        ...
        for (mapping in mappings) {
            ...
            val range =
                maxOf(remaining.first, mapping.range.first) + inc..minOf(remaining.last, mapping.range.last) + inc
            yield(range)
